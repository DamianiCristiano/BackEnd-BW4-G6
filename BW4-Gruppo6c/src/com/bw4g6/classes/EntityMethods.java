...
public class EntityMethods {
	...
	{
		Query q=em.createQuery("SELECT t.abbonamento FROM Tessera t WHERE t.id = :id AND (t.abbonamento.dataScadenza > CURRENT_DATE OR t.abbonamento.dataScadenza IS NULL)");
	...
	{
		Query q=em.createQuery("SELECT t.venditore.id, COUNT(t.id) FROM Ticket t WHERE t.dataAcquisto BETWEEN :x AND :y GROUP BY t.venditore");
		q.setParameter("x", x);
		q.setParameter("y", y);
		try {
			return q.getResultList();
